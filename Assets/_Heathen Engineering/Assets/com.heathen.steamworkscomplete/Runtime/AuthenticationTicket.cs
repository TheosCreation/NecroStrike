...
namespace HeathenEngineering.SteamworksIntegration
{
    ...
    public class AuthenticationTicket
    {
        ...
        public AuthenticationTicket(Action<AuthenticationTicket, bool> callback, bool isClient = true)
        {
            ...
            // Initialize SteamNetworkingIdentity if needed
            SteamNetworkingIdentity pSteamNetworkingIdentity = new SteamNetworkingIdentity();

            if (isClient)
            {
                // Make sure to fill in the SteamNetworkingIdentity if needed
                pSteamNetworkingIdentity.SetSteamID(SteamUser.GetSteamID());

                Handle = SteamUser.GetAuthSessionTicket(array, array.Length, out m_pcbTicket, ref pSteamNetworkingIdentity);
            }
            else
            {
                pSteamNetworkingIdentity.SetSteamID(SteamUser.GetSteamID());
                Handle = SteamGameServer.GetAuthSessionTicket(array, array.Length, out m_pcbTicket, ref pSteamNetworkingIdentity);
            }

            CreatedOn = SteamUtils.GetServerRealTime();

            // Resize the array to the actual ticket size
